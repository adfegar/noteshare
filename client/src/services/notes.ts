...
export async function addUserNote (addNoteRequest: AddNoteRequest): Promise<Note> {
  ...
  const note = addNoteResult.data as Note
  return note
}

export async function updateUserNote (noteId: number, newNote: UpdateNoteRequest): Promise<Note> {
  ...
  const note = updateNoteResult.data as Note
  return note
}
